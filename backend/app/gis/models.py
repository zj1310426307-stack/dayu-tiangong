...
class StructureResult(Base):
    ...
    native_applied_capacity: Mapped[float | None] = mapped_column(Float)
    actual_discharge: Mapped[float | None] = mapped_column(Float)
    intake_water_level: Mapped[float | None] = mapped_column(Float)
    outlet_water_level: Mapped[float | None] = mapped_column(Float)
    structure_head_difference: Mapped[float | None] = mapped_column(Float)
    pump_head: Mapped[float | None] = mapped_column(Float)
    pump_reduction_factor: Mapped[float | None] = mapped_column(Float)
    pump_actual_stage: Mapped[int | None] = mapped_column(Integer)
    transfer_type: Mapped[str | None] = mapped_column(String(24))
