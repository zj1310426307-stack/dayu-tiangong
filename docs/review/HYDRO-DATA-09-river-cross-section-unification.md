...
用户后续提供的 `gaominghe` 中心线截图已逐点整理为项目参考工作簿：21 点、桩号 `0–5432.1266 m`、几何折线累计长度约 `5432.1761 m`，最大累计差 `0.11285 m`。其 Branch 编码与断面表 `river_name=gaominghe` 一致。用户已确认中央经线 114°、中心线与断面均按上游到下游依次输入、断面采用局部零基准、所给中心线代表各断面最低点组成的纵向深泓线。因此 v03 联合导入文件使用 `EPSG:4547`、`flow_direction=forward`、`centerline_role=thalweg` 和 `vertical_datum=local_section_datum_0m`；原始高程不平移，权威 `bed_elevation_m` 保持未确认，未执行正式数据库提交。
...
- v03 工作簿反读：1 个 `thalweg` Branch、21 个中心线点、20 个 Profile、383 个断面点、21 个深泓候选点；错误 0，DM8 并列最低点警告 1 个，横向断面测线缺失警告 20 个。
