...
> **FIX1A 更新：** 当前工作流已切换到 `final-convergence-fix1a.json` 和 v3
> collector。v2/FIX1 Hosted runs 只保留为历史执行证据，不能替代 FIX1A 的 Python
> 3.11 与 Python 3.12 复跑。

> **FIX1 更新：** 下列 `8da24aa` Hosted runs 仅为 pre-FIX1 历史基线。当前工作流
> 后来曾切换到 `final-convergence-fix1.json` 和 v2 collector；这些证据现在也已由
> FIX1A 取代。
...
- JUnit 与 summary；
- `final-convergence-fix1a.json`（`dayu.d3a-final-convergence.v3`）；
...
FIX1A collector 只接受 `dayu.d3a-final-convergence.v3`、四层、`status=pass`、全部
`completion_gates=true`、fixed-monitor Q smooth convergence、global peak-Q non-smooth
分类以及显式 `13.99%` known limitation。旧 v1/v2 artifact 即使存在，也不能使当前
发布镜像科学门通过。
