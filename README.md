...
面向河网数字孪生的联合水动力、闸泵调度、多目标优化与 AI 辅助解释平台。现有 DGIS 负责 Web 展示、模型联动和服务发布；GIS-OPT-1 在不推翻该底座的前提下，引入 QGIS 3.44 LTR 作为专业桌面数据生产端，并用批次、质检、人工审核、不可变版本晋级和发布记录形成受控生产链。
...
编排顺序为 `database + redis + geoserver → migrate (0001…0012) → seed → qgis-bootstrap → app-bootstrap → dgis-bootstrap → geoserver-init → backend + worker → frontend`。`qgis-bootstrap` 幂等创建编辑、审阅和发布角色；`app-bootstrap` 创建非 owner 的 `dayu_backend` 运行账号并授予发布组成员资格；`geoserver-init` 创建只读数据库账号、`dayu` workspace、以 `publish` 为 schema 的 `dayu_postgis` store、12 个图层/SLD、`dayu_basemap` 图层组、Basic WFS 和 7 个 WMTS 缓存图层。
