...
def test_v4_task_accepts_optional_solver_assertion_and_requires_frozen_plan() -> None:
    ...
    with pytest.raises(ValidationError, match="solver assertion"):
        SimulationTaskCreate(
            case_id=71,
            input_schema_version="dayu.model-input.v4",
...
def test_openapi_exposes_rc2_build_and_delivery_contract() -> None:
    ...
    assert properties["retry_count"]["deprecated"] is True
    assert "infrastructure_retry_count" in properties
